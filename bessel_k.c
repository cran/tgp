 ...
 * needs to  allocate this each time.  Oh, and it takes the log.
 */

double log_bessel_k(double x, double nu, double exp0, double *bk, long nb)
{
  double alpha;
  long ize, ncalc;

  assert(bk && nb == floor(nu)+1);
  alpha = nu - (nb-1);
  ize = (long) exp0;

  K_bessel(&x, &alpha, &nb, &ize, bk, &ncalc);
  if(ncalc != nb) warning("bessel_k(%g,nu=%g): precision lost in result\n",
			  x, alpha+nb-1);
  return(log(bk[nb-1]));
}
 ...
 * K_bessel:
 *
 * copied directly from R's bessel_k.c
 */
