...
function(params, col)
{
  ...
  ## assign the base model (e.g., gp, or mrgp)
  if(params$base == "gp") { base <- 0; }
  else if(params$base == "mrgp"){ base <- 1; }
  else { cat(paste("params$base =", params$base, "not valid\n"));
         base <- 0; }
  ...
  ## initial settings of beta linear prior parameters for mrgp (base==1)
  else if(length(params$beta) != (2*(col-1)) && base==1) {
    cat(paste("length of params$beta should be", col-1, "you have", 
              length(params$beta), "\n"));
    return(NULL)
  }

  ## finally, set the params$beta for gp or mrgp
  ...
  ## mixture of gamma (initial) prior parameters for range parameter d
  ## for gp (base==0)
  else if(length(params$d.p) != 8 && params$base == "mrgp") {
    cat(paste("length of params$d.p should be 8 you have", 
              length(params$d.p),"\n"));
    return(NULL)
  }

  ## finally, set the params$d.p for gp or mrgp
  p <- c(p, as.numeric(params$d.p))

  ## delta.p -- only do this if we are using mrgp (base==1)
  ...
  ## nug.p -- only do this if we are using mrgp (base==1)
  ...
  if(params$corr == "matern") {
    if(params$nu < 0) {
      cat(paste("nu should be greater than zero, you have",  params$nu, "\n"))
      return(NULL)
    }
  }
