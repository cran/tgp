		   ...
		   double **bmu, double **Vb, double tau2, void *state)
{
  ...
  /* make the draw */
  double* new_nugs = mr_nug_draw_margin(n, col, nug, nugfine, X, F, Z, K, log_det_K, 
	*lambda, Vb, K_new, Ki_new, 
	...
	((MrExpSep_Prior*) prior)->Nugf_alpha(), ((MrExpSep_Prior*) prior)->Nugf_beta(),
	r, delta, gp_prior->s2Alpha(), gp_prior->s2Beta(), (int) linear, state);
			   ...
			   double *q_fwd, double *q_bak, void *state)
{
  *q_bak = *q_fwd = 1.0;
  ...
  for(unsigned int i=0; i<(2*nin); i++) b_new[i] = b[i];
		...
		double *lambda, double **bmu, double **Vb, double tau2, void *state)
{
  ...
  if(prior->Linear()) success = 1;
    ...
    if(success == 1) { /* could use swap_vector instead */
      ...
      linear = (bool) lin_new;
      for(unsigned int i=0; i<(2*nin); i++) b[i] = b_new[i];
  ...
  // if(deltasuccess) printf("delta = %g\n",delta);
