...
\caption{\footnotesize An example tree $\mathcal{T}$ with two splits,
  ...
  pictorially ({\em left}).  The notation $\mb{X}[:,u] < s$ represents
...
\cite{gra:lee:2006b}, where separate sequential $D$--optimal designs
...
({\tt R-1}).  The verbosity of progress meter print statements is
...
If you know that {\tt R} has already been linked to tuned linear
...
directly, one could re-compile all of {\tt R} linking it to {\tt
...
compared to the default {\tt R} linear algebra routines.  Comparisons
