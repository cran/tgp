...
\title{{\tt tgp} v1.2:\\ an {\tt R} package for Bayesian
...
the package, see the package help-manuals. At an {\tt R} prompt, type
...
The six regression models implemented in the package are summarized in
...
the function of interest into {\tt R}.  For example, to see the
...
\item The current version of this package contains no examples---nor
  ...
  large as 15 \cite{gra:lee:2006}, and were used in a sequential
...
predictions {\tt after} the MCMC has finished.  It gives the option of
obtaining predictive means and variances from the MAP
parameterization, $\hat{\bm{\theta}}$ maximizing $\pi(\bm{\theta}|Y)$,
saved in the {\tt tgp} class-output (from {\tt b*}).  In this way {\tt
  predict.tgp} is similar to {\tt predict.lm}, etc..  Samples can also
be obtained from the MAP--parameterized predictive distributions via
{\tt predict.tgp}, or a re--initialization of the joint sampling of
the posterior and posterior predictive distribution can commence
starting from the $\hat{\bm{\theta}}$.  The output of {\tt
  predict.tgp} is also a {\tt tgp} class object.  Appendix
\ref{sec:apred} illustrates how these features can be useful in the
context of passing {\tt tgp} model fits between collaborators.  There
are other miscellaneous demonstrations in Section \ref{sec:examples}.
...
sampling include {\tt Ds2x} and {\tt improv}.  Both are booleans, i.e.,
...
then $\Delta \sigma^2(\mb{\tilde{x}})$ statistic is computed at each
...
expected information gain, a.k.a, expected improvement (EI), for each
$\tilde{\mb{x}} \in \mbox{\tt XX}$ about the global minimum
\cite{jones:schonlau:welch:1998} .  The ALM (Active Learning--Mackay)
algorithm \cite{mackay:1992} is implemented by default in terms of
difference in predictive quantiles for the inputs {\tt XX}, which can
be accessed via the {\tt ZZ.q} output field.  Details on the ALM, ALC,
and EI algorithms are provided in Section \ref{sec:model}.
...
search good candidate configurations $\tilde{\mb{X}}$ for evaluating
the EI statistic, analogous to the branch and bound Expected Global
Optimization (EGO) algorithm \cite{jones:schonlau:welch:1998}, is
...
on Bayesian treed Gaussian process models \cite{gra:lee:2006}, and an
...
linear tend for data $D=\{\mb{X}, \mb{Z}\}$ where $m_X$ is the
number of covariates (columns) of the design matrix $\mb{X}$ and
$m\equiv m_X+1$.
\begin{align} 
...
\end{align} 
where $\mb{F} = (\mb{1}, \mb{X})$, and $\mb{W}$ is a $m
\times m$ matrix.  $N$, $IG$, and $W$ are the (Multivariate)
...
\begin{align}
  ...
  \nonumber
\end{align}
which is easily computed using MCMC methods \cite{gra:lee:2006b}.  In
...
\cite{gra:lee:2006}.  The key idea is an
...
graphics.off()
@
\includegraphics[trim=0 25 0 25]{tgp-gpllm}
%\vspace{-0.5cm}
...
be shown \cite{gra:lee:2006} that the predicted value of $z$ at
...
savings.  intense compared to ALC under a full GP.  Starting with the
predictive variance given in (\ref{eq:pred}), the expected reduction
in variance under the LM is \cite{gra:lee:2006b}
\begin{equation}
...
$\mathcal{T}$ \cite{gra:lee:2006}.  Integrating out dependence on
...
semiparametric (in the case of the GP LLM) regression tool.
...
For GPs with correlation matrix $\mb{K}$, this is equivalent to
maximizing det$(\mb{K})$.  Subsequently chosen input configurations
are called $D-$optimal designs.  Choosing quadratic loss leads to what
are called $A-$optimal designs.  An excellent review of Bayesian
approaches to the design of experiments is provided by Chaloner \&
Verdinelli~\cite{chaloner:1995}.
...
an active learning rule for determining the order in which they should
be considered for adding into the design.  The Active Learning--MacKay
(ALM) algorithm has been shown to approximate maximum expected
information designs by selecting the candidate location
$\tilde{\mathbf{x}}\in\tilde{\mb{X}}$ which has the greatest standard
deviation in predicted output \cite{mackay:1992}.  An alternative
algorithm is to select $\tilde{\mathbf{x}}$ minimizing the resulting
expected squared error averaged over the input space \cite{cohn:1996},
called ALC for Active Learning--Cohn.  Seo et al.~\cite{seo00} provide
a comparison between ALC and ALM using standard GPs. The EI
...
\cite{gra:lee:2006b}.  Separate sequential $D$--optimal designs can be
computed in each of the partitions depicted by the maximum {\em a
...
determinants.  This strategy tends to work well with ALM and ALC,
however it is less than ideal for EI as will be illustrated in Section
\ref{sec:as}.  Adaptive sampling from EI (with {\tt tgp}) is still an
...
applications to the Boston housing data \cite{harrison:78,
  gra:lee:2006}, and designing an experiment for a reusable NASA
launch vehicle \cite{glm:04,gra:lee:2006b} called the Langely
...
The progress meter shows the progress of the MCMC as it iterates
through the desired number of rounds of burn--in ({\tt BTE[1]}), and
sampling ({\tt BTE[2]-BTE[1]}), for the requested number of repeats
({\tt R}).  The verbosity of progress meter print statements is
...
algebra library different from the one used to compile {\tt R} (e.g.,
...
BLAS and LAPACK for fast, automatically tuned, linear algebra
routines.  Compared to standard {\tt BLAS}/{\tt Lapack}, those
automatically tuned by {\tt ATLAS} are significantly faster.  If you
know that {\tt R} has already been linked to tuned linear algebra
libraries (e.g., on {\tt OSX}), then compiling with {\tt ATLAS} as described
below, is unnecessary---just install {\tt tgp} as usual.  As an
alternative to linking {\tt tgp} to {\tt ATLAS} directly, one could
re-compile all of {\tt R} linking it to {\tt ATLAS}, or some other
platform--specific {\tt BLAS}/{\tt Lapack}, i.e., {\tt Intel}'s Math
Kernel Library, or {\tt AMD}'s Core Math Library, as described in:
\begin{center}
\verb!http://cran.r-project.org/doc/manuals/R-admin.html!
\end{center}
Look for the section titled ``Linear Algebra''. While this is
arguably best solution since all of {\tt R} benefits, the task can
...
instructions to disable {\tt ATLAS}. Don't forget to re-install the package
when you're done.
