...
\begin{figure}[ht!]
\centering
<<label=mapt,fig=TRUE,echo=TRUE,width=6,height=5,include=FALSE>>=
tgp.trees(exp1)
@
<<echo=false,results=hide>>=
graphics.off()
@
\includegraphics[trim=50 50 50 20]{as-mapt}
\vspace{-1cm}
...
\begin{figure}[ht!]
\centering
<<label=cands,fig=TRUE,echo=TRUE,width=5,height=5,include=FALSE>>=
...
graphics.off()
@
\includegraphics[trim=0 0 0 45]{as-cands}
\vspace{-0.5cm}
...
\begin{figure}[ht!]
\centering
<<label=expas,fig=TRUE,echo=TRUE,width=12,height=4,include=FALSE>>=
...
\includegraphics[trim=75 0 75 20]{as-expas}
\vspace{-0.5cm}
\caption{\footnotesize {\em Left}: Image plots of adaptive sampling
