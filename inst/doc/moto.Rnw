...
input-dependent noise which makes it useful for illustrating how the
treed GP model handles this nuance.  There are at least two---perhaps
three---three regions where the response exhibits different behavior
both in terms of the correlation structure and noise level.
...
functions automates this procedure.  Progress indicators
are surpressed.
\begin{figure}[ht!]
\centering
...
\label{f:moto:tgp}
\end{figure}
The resulting posterior predictive surface is shown in the {\em top}
half of Figure \ref{f:moto:tgp}.  The {\em bottom} half of the figure
shows the norm (difference) in predictive quantiles, clearly
illustrating the treed GP's ability to capture input-specific noise in
