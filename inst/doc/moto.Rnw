...
range of one.  The {\tt m0r1} argument to {\tt b*} functions automates
this procedure.  Progress indicators are suppressed.
\begin{figure}[ht!]
\centering
...
graphics.off()
@
\includegraphics[trim=0 25 0 25]{moto-bgp}
%\vspace{-0.5cm}
...
graphics.off()
@
\includegraphics[trim=0 25 0 25]{moto-btlm}
%\vspace{-0.5cm}
\caption{\footnotesize Posterior predictive distribution using {\tt
...
\begin{figure}[ht!]
\centering
<<label=btgp,fig=TRUE,echo=TRUE,width=8,height=4,include=FALSE>>=
par(mfrow=c(1,2))
...
\includegraphics[trim=50 25 50 20]{moto-btgp}

<<label=btgpq,fig=TRUE,echo=TRUE,width=8,height=4,include=FALSE>>=
par(mfrow=c(1,2))
...
\includegraphics[trim=50 25 50 20]{moto-btgpq}
...
$\bm{\beta}_0$ of $\mb{W}^{-1}$ in (\ref{eq:model}).  This was chosen
...
hierarchical \verb!bprior="b0"!  or the independent
\verb!bprior="b0tau"! might be preferred.
