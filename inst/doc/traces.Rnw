%\VignetteIndexEntry{working with parameter traces}
...
product of $|${\tt XX}$|$, \verb!(BTE[2]-BTE[1])/BTE[3]! and {\tt R}
...
files are read into {\tt R} and stored as {\tt data.frame} objects,
...
out$trace
@
\caption{\footnotesize Listing the contents of
  {\tt "tgptraces"}--class objects.}
\label{f:tgptraces}
\end{figure} 
...
\begin{figure}[ht!]
\centering
<<label=XXd,fig=TRUE,echo=TRUE,include=FALSE,width=8,height=5>>=
...
graphics.off()
@
\includegraphics[trim=55 25 65 20]{traces-XXd}
\caption{\footnotesize Traces of the (log of the) first range
...
\begin{figure}[ht!]
\centering
<<label=la,fig=TRUE,echo=TRUE,include=FALSE,width=8,height=6>>=
hist(out$trace$linarea$la)
@
<<echo=false,results=hide>>=
graphics.off()
@
\includegraphics[trim=0 0 0 20]{traces-la}
\vspace{-0.5cm}
\caption{\footnotesize Histogram of proportions of the area of the input 
...
\begin{figure}[ht!]
\centering
<<label=alc,fig=TRUE,echo=TRUE,include=FALSE,width=8,height=5>>=
...
graphics.off()
@
\includegraphics[trim=55 25 65 20]{traces-alc}
\caption{\footnotesize Traces of the (log of the) samples
