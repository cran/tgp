%\VignetteIndexEntry{collaborating with predict.tgp}
...
data, and suppose that we do inference for the model parameters only
(obtaining no samples from the posterior predictive distribution.  The
...
\begin{figure}[hp]
\centering
<<label=kp,fig=TRUE,echo=TRUE,include=FALSE,width=8,height=4>>=
...
@
\vspace{-0.1cm}
\includegraphics[trim=50 30 50 25]{pred-kp}
<<label=p,fig=TRUE,echo=TRUE,include=FALSE,width=8,height=4>>=
plot(out.p)
@
<<echo=false,results=hide>>=
graphics.off()
@
\vspace{-0.1cm}
\includegraphics[trim=50 30 50 25]{pred-p}
<<label=2,fig=TRUE,echo=TRUE,include=FALSE,width=8,height=4>>=
plot(out2)
@ 
<<echo=false,results=hide>>=
graphics.off()
@
\includegraphics[trim=50 30 50 25]{pred-2}
\caption{\footnotesize Predictive surfaces ({\em left}) and
...
of the three calls to {\tt predict.tgp} above.  The kriging surfaces are
smooth within regions of the partition, but the process is
...
predictive distribution when obtained jointly with
...
The {\tt predict.tgp} function can also sample from the ALC statistic and
calculate expected improvements (EI) at the {\tt XX} locations.  
