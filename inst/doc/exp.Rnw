...
Linear CART is clearly just as inappropriate for this data as it was
...
the large uninteresting oart of the input space from the interesting part.
...
\cite{gramacy:thesis:2005}. Also, Chipman et al.~recommend restarting 
...
is active.  Figure \ref{f:exp:btgpllm} show how similar the resulting
posterior predictive surfaces are compared to the treed GP (without
LLM).

\begin{figure}[ht!]
\centering
<<label=1dbtgpllm1,fig=TRUE,echo=TRUE,width=12.5,height=7.5>>=
plot(exp.btgpllm, main='treed GP LLM,', proj=c(1))
@
<<echo=false,results=hide>>=
dev.off()
@
<<label=1dbtgpllm2,fig=TRUE,echo=TRUE,width=12.5,height=7.5>>=
...
indicate the mean of posterior priedictive distribution.  Red lines
